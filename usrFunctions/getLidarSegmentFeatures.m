function nodeSegmentFeatures = getLidarSegmentFeatures(ptCloud, normHeight, nodeLabels)
%% Features
...
[avrSegRH, varSegRH, maxDiffSegRH, minDiffSegRH] = getRelativeHeight(avrSegmentNormHeight, neighSeg_cell);
...
nodeSegmentFeatures = [avrSegmentNormHeight(U), varSegmentNormHeight(U), ...
    avrAngSegNormals(U), varAngSegNormals(U)]; %%, avrSegRH(U), varSegRH(U), ...
%     maxDiffSegRH(U), minDiffSegRH(U)];
...
% Compute relative height
    function [avrSegRH, varSegRH, maxDiffSegRH, minDiffSegRH] = getRelativeHeight(avrSegmentHeight, neighSeg_cell)
        ...
        varSegRH = cellfun(@(nCell, idx) var(arrayfun(@(idxNeig) abs(avrSegmentHeight(idx) - ...
            avrSegmentHeight(idxNeig)), nCell)), neighSeg_cell, idx_cell);
        maxDiffSegRH = cellfun(@(nCell, idx) max(arrayfun(@(idxNeig) abs(avrSegmentHeight(idx) - ...
            avrSegmentHeight(idxNeig)), nCell)), neighSeg_cell, idx_cell);
        minDiffSegRH = cellfun(@(nCell, idx) min(arrayfun(@(idxNeig) abs(avrSegmentHeight(idx) - ...
            avrSegmentHeight(idxNeig)), nCell)), neighSeg_cell, idx_cell);
    end
